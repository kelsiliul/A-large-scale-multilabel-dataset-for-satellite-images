...
parser.add_argument('-od', '--output-dir', help='The name of the output directory', default='saved')
...
# Choose sampling centroid that satisfy: Multilabel, Good polygon Coverage
multicoords2=[]
counter=0
savedpolygons=[]
savedpolygons2={}
...
npz_file_path = join(args.output_dir, "{}.npz".format(args.input_file.split('/')[-1].split('.')[0]))
np.savez(npz_file_path, keys=keys, values=values)

# Save Sampling Centroid
with open(join(args.output_dir, '{}.csv'.format(args.input_file.split('/')[-1].split('.')[0])), 'w') as ofd:
    writer = csv.writer(ofd)
